...
function isAuthorised(req: NextRequest) {
  ...
  // Manual trigger via Authorization header
...
export async function POST(req: NextRequest) {
  // Allow admin-triggered refresh from the UI (checks session via requireAdmin)
  ...
  return runScrape(body.competitor_id ?? null)
}
...
async function runScrape(competitorId: string | null) {
  ...
  try {
    ...
    return NextResponse.json({
      message: `Scrape complete: ${results.success} success, ${results.errors} errors, ${results.alerts} alerts sent`,
      ...results,
    })
  } catch (err: unknown) {
